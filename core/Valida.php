...
class Valida
{
     ...
     */
    public function set($name, $value, $value2 = null){
        $this->_data['name'] = $name;
        $this->_data['value'] = $value;
        $this->_data['value2'] = $value2;
        return $this;
    }
    ...
    protected function set_messages_default(){
        $this->_messages = array(
            ...
            'is_match'       => 'O campo %s está diferente do campo %s', 
            'is_unique'      => 'Este %s já está cadastrado em nossos sistema!', 
    ...
    public function is_match($campo)
    {
        if ($this->_data['value'] != $this->_data['value2']) {
            $this->set_error(sprintf($this->_messages['is_match'], $this->_data['name'], $campo));
        }
        return $this;
    }

     public function is_unique($campo) {
        if ($this->model::where($campo, $this->_data['value'])->first()) {
            $this->set_error(sprintf($this->_messages['is_unique'], $this->_data['name']));
        }
        return $this;
    }
